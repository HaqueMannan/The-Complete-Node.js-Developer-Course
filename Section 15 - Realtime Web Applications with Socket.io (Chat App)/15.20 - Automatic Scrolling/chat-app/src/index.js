...
// SOCKET.IO JOIN:
// The .join() method can only be used on the server.
// The .join() allows us to join a given chat room and we passed to it the name of the room we are trying to join. 
// This is going to provide us a whole new way to emit events to specifically to that room. Therefore, only others in that room would actually see the emitted event messages or anything else that is sent from server to client.
// Rooms introduces two new emit events which is a variation of io.emit() and socket.broadcast.emit() above:
//    1) io.to().emit() emits the event to every single connected client within a room.
//    2) socket.broadcast.to().emit() emits the event to every other connected client except the current 
//       client within a room.
// The .to() is a function and we pass to it a string which is the room we are trying to emit that event to.

// SOCKET.IO TRACKING ID:
// Each socket has a unique id which we can access using the socket.id method. This will return the id for that particular connection.
// The socket is in the parent scope which means inside of each child scopes we have access to the socket.id which allows us to retrieve the user object from the users array via the socket.id which allows for tracking data.